...
public class reading extends AbstractApplication {
	...
	public Object read(String bookId, int chapterId, int sectionId) throws ApplicationException {
		if(bookId == null) 		throw new ApplicationException("Invalid book Id");
		...
		try {
			...
			Table table = book.findWith("WHERE id=? and language=?",
		...
		this.maxChapter = article.setRequestFields("max(chapter_id) as max_chapter").findWith("WHERE book_id=?",
				new Object[] { this.bookId }).get(0).get(0).get("max_chapter").intValue();
		this.lastChapterId = this.chapterId - 1 <= 0 ? 1 : this.chapterId - 1;
		this.nextChapterId = this.chapterId + 1 > this.maxChapter ? this.maxChapter
				: this.chapterId + 1;
				...
				while (iterator.hasNext()) {
					...
					buffer.append("<li"
								+ (this.sectionId == fields.get("section_id").intValue() ? " class=\"selected\""
										: "")
								+ "><a class=\"sup\">" + fields.get("section_id").intValue()
								+ "</a>" + finded + "</li>");
				}
			}
			buffer.append("</ol>");
	...
	public Object book(String bookId,int chapterId,int sectionId) throws ApplicationException {
		...
		xml.append("<book id=\"book\" name=\"book\" bookId=\"" + this.bookId
				+ "\" bookname=\"" + this.book.getName() + "\" chapterId=\""
				+ this.chapterId + "\" maxchapter=\"" + this.maxChapter
		...
		for (Enumeration<Row> table = vtable.elements(); table
			...
			while (iterator.hasNext()) {
				fields = iterator.next();
				finded = fields.get("content").value().toString();
				if (this.sectionId == Integer.parseInt(fields.get("section_id")
						.value().toString())) {
					xml.append("<item uid=\""
							+ fields.get("id").value().toString() + "\" id=\""
							+ fields.get("section_id").value().toString()
	...
	private Table load(String bookId,int chapterId,int sectionId) throws ApplicationException {
		...
		this.maxChapter = article.setRequestFields(
	...
	public static void main(String[]args) throws ApplicationException, IOException{
		...
		String s = "<div id=\"postmessage_(.+?)\" class=\"t_msgfont\">(.+?)(</div>)"; //<div id=\"postmessage_(.*)\" class=\"t_msgfont\">(.*)</\1>(.*)
